...
{
    private const ACCOUNT_PREFIX = 'RTR';
    private const ACCOUNT_START_SEQUENCE = 24000;
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'data' => [
                ...
                'account_id_format' => 'RTR-{TEAM_CODE}-#####',
                'filters' => [
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $rescuer = DB::transaction(function () use ($request, $validated): array {
            ...
            DB::table('responders')->insert([
                ...
                'user_id' => $userId,
                'responder_code' => ($validated['responder_code'] ?? null) ?: $this->nextResponderCode($teamId),
    ...
    {
        $rules = [
            'account_id' => ['nullable', 'string', 'max:30', 'regex:/^RTR-[A-Z0-9]{2,8}-[0-9]{5}$/i'],
            'responder_code' => ['nullable', 'string', 'max:80'],
        ...
        $messages = [
            'account_id.regex' => 'Rescuer Account ID must follow the DB format RTR-SAR-24001.',
    ...
    private function nextAccountId(?string $teamCode = null): string
    {
        ...
        $pattern = '^'.self::ACCOUNT_PREFIX.'-'.$teamCode.'-[0-9]{5}$';
        ...
        return self::ACCOUNT_PREFIX.'-'.$teamCode.'-'.str_pad((string) ($maxSequence + 1), 5, '0', STR_PAD_LEFT);
    }

    private function accountSequence(?string $accountId, string $teamCode): int
    {
        if (! $accountId || ! preg_match('/^RTR-'.preg_quote($teamCode, '/').'-([0-9]{5})$/i', $accountId, $matches)) {
            return 0;
        }
    ...
    {
        return preg_match('/^RTR-'.preg_quote($teamCode, '/').'-[0-9]{5}$/i', $accountId) === 1;
    }
    ...
    private function nextResponderCode(?int $teamId): string
    {
        ...
        return 'BDRRM-'.$prefix.'-'.str_pad((string) $count, 3, '0', STR_PAD_LEFT);
    }
